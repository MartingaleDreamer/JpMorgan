...
def find_max_epoch(path):
    ...
    epoch = -1
    for f in files:
        if len(f) <= 3:         # 这里在检查文件路径下的文件夹每个文件名字的长度，如果文件名长度都不够4个(或等于4个)字符则跳出
            continue
        if f[-3:] == '.h5':      # 这里检查在所有文件名长度大于4个字符的文件中，文件名末尾4个是否是.pkl，即checkpoint文件，如果是，测查看epoch数值大小(和-1比大小取更大值)，E.g. 100000.pkl，并最终返回
            try:
...
def calc_diffusion_hyperparams(T, beta_0, beta_T):
    ...
    for t in range(1, T):
        ...
        # / (1-\bar{\alpha}_t)
    Sigma = torch.sqrt(Beta_tilde)  # \sigma_t^2  = \tilde{\beta}_t                     # [Zihao]: torch used
