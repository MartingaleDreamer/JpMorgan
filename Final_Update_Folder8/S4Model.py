...
class S4(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        if verbose:        # 这里是废的，因为src里面有utils但是utils只是一个文件夹，文件夹里面是util.py，其里面也没有train函数，这里没有使用是因为verbose = False
        ...
        #self.D = tf.Variable(tf.random.normal([channels, self.h],mean=0,stddev=1))
    ...
    def forward(self, u, **kwargs): # absorbs return_output and transformer src mask
        ...
        # u = torch.from_numpy(u.numpy())  # Add
...
class S4Layer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute((1,2,0))  #x = tf.transpose(x,perm=[1,2,0]) #batch, feature, seq (as expected from S4 with transposed=True)
        xout, _ = self.s4_layer(x) #batch, feature, seq
        xout = self.dropout(xout)
        xout = xout + x # skip connection   # batch, feature, seq
        xout = xout.permute((2,0,1))  # xout = tf.transpose(xout,perm=[2,0,1]) # seq, batch, feature
